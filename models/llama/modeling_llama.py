...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        #     attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
            ...
            #     if sparsity_ratio > 0 and sparsity_ratio < 1: 
            #         num_elements = attn_weights.shape[2] * attn_weights.shape[3]
            #         num_to_keep = int(num_elements * (1 - sparsity_ratio))

            #         batch_size, num_heads, q_len, seq_len = attn_weights.shape
            #         attn_weights_flat = attn_weights.view(batch_size, num_heads, q_len * seq_len)
            #         _, indices = torch.topk(attn_weights_flat, num_to_keep, dim=-1, largest=True, sorted=False)
            #         mask = torch.zeros_like(attn_weights_flat, dtype=torch.bool)
            #         mask.scatter_(2, indices, True)

            #         attn_weights_flat = attn_weights_flat * mask
            #         attn_weights = attn_weights_flat.view(batch_size, num_heads, q_len, seq_len)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        # print(config._attn_implementation) # sdpa
